...
class DatabaseHelper {
  ...
  // Menggunakan prefix 'model.'
  // Menambahkan fungsi untuk menampilkan transaksi yang total harganya lebih besar dari total harga yang diinputkan
  Future<List<model.Transaction>> getTransactionsByTotalAmount(int totalAmount) async {
    final db = await instance.database;
    final result =
        await db.query('select * from transactions where totalAmount > $totalAmount');
    return result.map((json) => model.Transaction.fromMap(json)).toList();
  }
